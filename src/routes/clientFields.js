 ...
 *   get:
 *     tags: [Client Fields]
 *     summary: List custom client-card field definitions for the caller's company
 *     security: [{ bearerAuth: [] }]
 *     parameters:
 *       - in: query
 ...
 *   post:
 *     tags: [Client Fields]
 *     summary: Add a new custom field — applies to every client card in the company (admin only)
 *     security: [{ bearerAuth: [] }]
 *     requestBody:
 *       content:
 *         application/json:
 ...
 *               options: { type: array, items: { type: string }, description: "Required for field_type=select" }
 *     responses:
...
router.get('/', async (req, res) => {
  ...
  const { rows } = await pool.query(
    `SELECT * FROM company_client_fields WHERE company_id = $1 ${includeInactive ? '' : 'AND active = TRUE'} ORDER BY sort_order, created_at`,
    [req.auth.companyId]
  );
  res.json(rows);
});
  ...
  async (req, res) => {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 20; attempt++) {
        const exists = await client.query('SELECT 1 FROM company_client_fields WHERE company_id = $1 AND key = $2', [req.auth.companyId, key]);
        if (!exists.rows.length) break;
        key = `${slugifyKey(label)}_${attempt + 2}`;
      }
      const maxOrder = await client.query('SELECT COALESCE(MAX(sort_order), 0) AS m FROM company_client_fields WHERE company_id = $1', [req.auth.companyId]);
      const { rows } = await client.query(
        `INSERT INTO company_client_fields (company_id, key, label, field_type, options, sort_order)
...
router.patch('/:id', requireRole('admin'), async (req, res) => {
  ...
  const { rows } = await pool.query(
    `UPDATE company_client_fields SET
