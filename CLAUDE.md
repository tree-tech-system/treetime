...
- **שינוי גודל widgets — 3 ידיות מתיחה בכיוונים מפורשים, לא גדלים קבועים (17.8.2026 → 4 גדלים קבועים ב-18.8.2026 → 2 חצי מתיחה ב-18.8.2026 → 3 ידיות כיוון מפורשות, 20.8.2026).** נוסה גם resize חופשי בפיקסלים וגם 4 גדלים קבועים (¼/½/¾/מלא); שניהם הוחלפו בעקבות בקשה מפורשת בחזרה למתיחה חופשית, דרך שתי ידיות בצורת חץ (לא נקודת גרירה בפינה, ולא כפתורי גודל) — רוחב וגובה. **20.8.2026: ידית הרוחב היחידה (עם היפוך כיוון לפי RTL, ניחוש) פוצלה לשתי ידיות מפורשות** לפי בקשה מפורשת ("3 אפשרויות: מתיחה ימינה, מתיחה שמאלה, מתיחה למטה", בהשראת UX של אוריגמי) — `.widget-resize-arrow-w-right` (קצה ימין) ו-`.widget-resize-arrow-w-left` (קצה שמאל), שתיהן `↔`/`cursor:ew-resize`, בנוסף ל-`.widget-resize-arrow-h` (↕, מרכז תחתון, `cursor:ns-resize`) הקיימת למתיחת **גובה** — 3 ידיות סה"כ. הכיוון נגזר מ-`e.currentTarget.classList` בזמן ה-mousedown (לא מ-document direction) ונשמר על ה-state של הגרירה הפעילה: ידית ימין — גרירה ימינה מגדילה; ידית שמאל — גרירה שמאלה מגדילה (סימן הפוך, לא תלוי RTL בכלל יותר — ביטול הניחוש הישן). לחיצה והחזקה על ידית ואז גרירה, אותה פונקציית handler משותפת לשתי ידיות הרוחב (`onWidgetWidthResizeStart/Move/End`), גובה נשאר בפונקציות נפרדות משלו (`onWidgetHeightResizeStart/Move/End`). כל השלוש שומרות ב-`PATCH /api/dashboard-widgets/:id` (`width_px`/`height_px`) בסיום הגרירה, מינימום 200px רוחב / 60px גובה. עמודת `size` מ-הניסיון הקודם (migration 025) נשארה בטבלה כשריד לא מזיק, לא נקראת יותר — כמו `width_px` בזמנו. המרחק בין widgets נשאר אחיד תמיד — `gap:12px` על ה-container הגמיש (`flex-wrap`), לא תלוי בגודל הפרטני של כל widget.
  - **תיקון נלווה, אותו יום: הצד הנגדי לידית שנגררת חייב להישאר מקובע — לא היה כך בהתחלה. עבר גרסה שגויה לפני שהתייצב על הפתרון הנכון.** בדף RTL הזה, שינוי `width` בלבד על פריט flex תמיד משאיר את קצה **ימין** שלו מקובע (נקבע ע"י סכום הרוחב של האחים הקודמים ב-row, לא מושפע מהרוחב העצמי) ומותח את קצה **שמאל**. זה בדיוק ההתנהגות הרצויה לידית **שמאל** — אין צורך בהתערבות. לידית **ימין** זה הפוך ממה שרוצים, אז נדרש `transform:translateX()` מפצה על הקופסה עצמה.
    ...
    - **המחיר המוסכם (החלטת מוצר מפורשת מהמשתמש, אבל בוטל באותו יום אחרי שהמשתמש ראה את זה בפועל בצילום מסך — widget גלש ממש לתוך ה-sidebar הכהה).** בהתחלה: אם אין מספיק מקום פנוי בשורה, האחים שנדחפים עלולים לגלוש ויזואלית מעבר לקצה הדשבורד — טרייד-אוף מקובל מול חפיפה, בלי מנגנון מניעת-גלישה. **בוטל:** המשתמש ראה בפועל שזה נראה שבור (widget בולט לתוך העמודה הירוקה של הניווט) ודרש "שהוא ישאר בגבולות גזרה שלו ושוויג׳טים לא יעלו אחד על השני אף פעם בשום צורה" — ראו bullet הבא ל-clamp שנוסף בעקבות זה.
    - **Clamp על גבול ה-container, אותו יום.** `maxGrow` מחושב ב-`onWidgetWidthResizeStart` (ידית ימין בלבד): `container = el.parentElement` (ה-`.panel-pad`), `containerRight = container.getBoundingClientRect().right - paddingRight` (גבול ה-content, לא כולל padding), ואז `Math.max(0, containerRight - rightmostEdge)` כש-rightmostEdge הוא ה-`right` הגבוה ביותר מבין ה-widget עצמו וכל ה-rowMates שלו (נמדד **לפני** הגרירה). ב-`onWidgetWidthResizeMove`, ה-width עצמו (לא רק ה-transform) נחסם ב-`Math.min(w, startW + maxGrow)` **לפני** ה-`Math.max(200,...)` — כך שגם ה-`width_px` שבסוף נשמר ב-DB לא "ידלוף" מעבר למה שהיה ויזואלי מותר, לא רק ה-transform. נמדד fresh בכל mousedown (לא מטמון) — כך שגרירות חוזרות/מעבר בין ידיות מחשבות נכון כמה מקום *עוד* נשאר, לא כמה היה בהתחלה.
    - **תובנה מבנית חשובה שהתגלתה תוך כדי אימות ב-Playwright אמיתי (לא הייתה ברורה מראש): תחת `justify-content:flex-start` (ברירת המחדל של האפליקציה, לא נדרס אף פעם — נבדק), ה-widget הראשון/ימני-ביותר בשורה **תמיד** צמוד לגמרי לגבול ה-content של ה-container, בלי קשר לכמה "מקום פנוי" יש בשורה בסך הכל.** מקום פנוי (מ-flex-grow שלא מנוצל, מ-padding, מריכוז ה-widgets) תמיד "נוחת" בקצה השני (שמאל) — **לעולם לא** לפני הפריט הראשון. נבדק ונדחה גם עם `padding-right` גדול על ה-container וגם עם `justify-content:flex-end` (שני ניסיונות נכשלו/לא רלוונטיים כדי ליצור "מקום פנוי אמיתי" ליד הימין) לפני שהובן שזו תכונה מבנית של flex-start ולא באג במדידה. **המשמעות המעשית: בשורה "מלאה" טיפוסית (widgets שממלאים אותה, המצב השכיח ביותר) ה-clamp יאפשר רק כמות זעירה של גדילה אמיתית לימין** — לא כי ה-clamp שגוי, אלא כי גאומטרית פשוט אין עוד מקום בלי לכווץ שכן קיים (יכולת שלא התבקשה ולא נבנתה). זה הצפוי וה"נכון" גיאומטרית, אבל שווה לדעת אם המשתמש ירגיש שהידית "לא מגיבה" ברוב המקרים בפועל.
    ...
    - **אומת עם Playwright אמיתי (לא VM sandbox מדומה) בשני מצבים:** (1) שורה עם מקום פנוי אמיתי (fixture ייעודי ל-בדיקה עם `margin-right` מכוון על ה-widget הראשון, כדי ליצור "תקציב" נמדד של 90px — לא טענה שכך נראית שורה אמיתית, רק דרך תקפה תחת flex-start ליצור מקום פנוי מדיד): גרירת ימין +60px → אח קודם נדחף +60 בדיוק, ה-widget גדל נכון (קצה שמאל קבוע, ימין +60), האח שאחרי **לא זז כלל**, הרווח של 12px נשמר משני הצדדים; גרירה חוזרת (+30 נוסף) מכלה בדיוק את שאר התקציב (90 total, בלי קפיצה); גרירה שלישית שמנסה עוד +100px נחסמת כמעט לגמרי (התקציב נגמר) בלי חפיפה. ריענון + גרירת ידית שמאל → בדיוק כמו לפני (אח קודם לא מושפע, אח שאחרי נדחף ע"י flow טבעי, בלי transform וללא clamp — לא נדרש). (2) שורה **בלי** מקום פנוי בכלל (בדיוק התרחיש מצילום המסך של המשתמש — 2 widgets תופסים את כל השורה): גרירת ימין של 500px (הרבה מעבר לכל מקום סביר) → אפס תזוזה, אפס חפיפה, אפס גלישה מעבר לגבול ה-container — בדיוק ההתנהגות הנדרשת.
