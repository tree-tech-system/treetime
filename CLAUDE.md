...
- **שינוי גודל widgets — הוחלף מגרירה חופשית לרוחב ל-4 גדלים קבועים (17.8.2026 → עודכן 18.8.2026).** בהתחלה נבנה resize חופשי בפיקסלים (`width_px`); הוחלף בעקבות בקשה מפורשת ל-4 גדלים בלבד: רבע/חצי/שלושת-רבעי/מסך מלא (`quarter`/`half`/`three_quarter`/`full`), נשמר בעמודת `size` (migration 025, TEXT, `NULL`=ברירת מחדל — quarter ל-KPI, full לרשימה, תואם ויזואלית למה שהיה לפני). בכל widget לאדמין יש size-picker (4 כפתורי קיצור ¼/½/¾/⛶, `renderWidgetSizePicker`/`setWidgetSize`) שקובע רוחב דרך קלאסים קבועים ב-CSS (`.widget-size-quarter` וכו', עם `calc()` שמפצה על ה-`gap`, וקורס לרוחב מלא מתחת ל-720px). **גובה** נשאר גמיש חופשי בעכבר — ידית resize (`.widget-resize-handle`, פינה תחתונה, `cursor:ns-resize`) גוררת רק אנכית ושומרת `height_px` ב-`PATCH` בסיום הגרירה (`onWidgetResizeStart/Move/End`). עמודת `width_px` הישנה (migration 024) נשארה בטבלה כשריד לא מזיק, לא נקראת יותר. המרחק בין widgets נשאר אחיד תמיד — `gap:12px` על ה-container הגמיש (`flex-wrap`), לא תלוי בגודל הפרטני של כל widget.
- **שינוי סדר widgets בגרירה, בנוסף לחצים (18.8.2026).** לחיצה ארוכה (350ms, `mousedown` בלי תזוזה של יותר מ-6px) על widget (לא על כפתור/select/ידית resize) מתחילה גרירה (`onWidgetDragMouseDown`/`Move`/`Up`) — הזזת העכבר מסדרת מחדש את ה-DOM חי מול שאר ה-widgets (`document.elementFromPoint` + `insertBefore` לפי חצי שה-cursor נמצא בו), שחרור העכבר שומר את הסדר החדש דרך אותו endpoint קיים `POST /dashboard-widgets/reorder`. חצי ↑/↓ עדיין קיימים במקביל (לא הוסרו) כדרך נגישה חלופית. הגרירה מוגבלת ל-widgets בפועל בלבד (`data-draggable="1"`, אדמין בלבד) — לא חוצה לפאנלים הקבועים (ראו הבא).
