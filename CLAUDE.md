...
- **`GET /api/search/fields` (נוסף בהמשך אותו יום) — איחוד כל השדות מכל האזורים, בלי תלות ב-entity.** נוסף אחרי שהתברר בפועל, תוך כדי בניית מודול Search ב-Make, שאין דרך אמינה להעביר ערך של פרמטר-אח (`entity`) לתוך RPC שמופעל משדה **מקונן** בתוך פרמטר מסוג `array` (כמו "שדה" בתוך שורת "תנאי חיפוש") — כל תחביר שנוסה החזיר `{{parameters.entity}}` ריק. הפתרון: `listAllFields()` מחזיר union מדודופלק של כל 21 השדות מכל 6 האזורים (לפי `key`, הראשון שמגדיר כל key מנצח), כדי שה-RPC הזה יהיה **פרמטר-פרי** בדיוק כמו `listEntities` שכבר עבד — במקום להמשיך לנחש את תחביר ה-dependent-RPC של Make. זה בטוח כי `buildWhereClause`/`POST .../query` עדיין דוחים כל שילוב שדה+אזור לא תואם עם `unknown_field` — כלומר אין באמת סיכון אבטחתי או פונקציונלי ב"להציע יותר מדי" שדות, רק חוסך את הצורך לפצח תלות בין פרמטרים ב-Make.
