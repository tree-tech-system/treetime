...
| `ownerEmail.js` | הגדרות SMTP + שליחת מייל לכל האדמינים בפלטפורמה (ראו "אינטגרציית מייל" למעלה) |
...
- **שליחת מייל לכל האדמינים בפלטפורמה (18.8.2026).** `POST /api/owner/email/broadcast` — `{subject, body}`, נשלח ל-`SELECT email FROM employees WHERE role='admin' AND active=TRUE` **בלי סינון company_id בכוונה** (owner-level = כל הפלטפורמה, כמו `ownerChangelog`/`ownerNotifications`). דרך `Promise.all` בלי batching/rate-limiting — מספיק בהיקף הנוכחי (~19 חברות), אם יגדל משמעותית ייתכן שיידרש queue. אין אישור/confirm בצד השרת — ה-UI עושה `confirm()` לפני השליחה כי זו פעולה בלתי הפיכה (מייל אמיתי לכל הלקוחות), אין לוג/היסטוריה נשמרת של broadcasts שנשלחו.
- **מה עוד לא הוגדר / נשאר לעשות:** **אין עדיין ספק SMTP אמיתי מוגדר** — המשתמש בחר ספק בראש אבל עוד לא סיפק host/port/username/password. עד אז אין ערך ב-DB או ב-`.env` והמערכת רק רושמת ל-log במקום לשלוח בפועל (ראו "TODO פתוח"). כשיהיו פרטים: להזין אותם ישירות דרך פאנל ה-owner (✉️ מייל) — לא צריך יותר SSH ל-`.env` לשם כך, אבל האופציה עדיין קיימת כברירת מחדל/fallback. גם צריך רשומות DNS (SPF/DKIM) בדומיין כדי שלא ייפול לספאם.
