...
- **שינוי גודל widgets בעכבר (17.8.2026).** ידית resize (`.widget-resize-handle`, פינה תחתונה) בכל widget לאדמין; גרירה מעדכנת `width_px`/`height_px` בזמן אמת ושומרת ב-`PATCH /api/dashboard-widgets/:id` בסיום הגרירה (`wireDashboardWidgetResize`/`onWidgetResizeStart/Move/End`). עמודות `width_px`/`height_px` ב-`dashboard_widgets` (migration 024, `NULL` = גודל ברירת מחדל לפי flex). המרחק בין widgets נשאר אחיד תמיד — `gap:12px` על ה-container הגמיש (`flex-wrap`), לא תלוי בגודל הפרטני של כל widget, ולא נפתר drag & drop למיקום (סדר עדיין רק חצים).
