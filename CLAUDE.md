...
### התראה על דיווח עבודה ארוך — סף דקות ניתן להגדרה (20.8.2026)

הרחבה חדשה ל"הרשאות התראות" הקיימות: האדמין יכול להגדיר סף דקות (למשל 120), וכל דיווח עבודה בודד שמגיע אליו או חוצה אותו מקפיץ התראה — לא רק on/off כמו שאר סוגי ההתראות בטבלה, אלא ערך מספרי שהאדמין קובע בעצמו.
...
- **UI:** שורה רביעית ב-"הרשאות התראות" (`renderNotificationSettingsPanel`, index.html) — לא דרך ה-helper `row()` הגנרי הקיים (שתומך רק בתוויות טקסט קבועות), כי השורה הזו צריכה קלט מספרי בתוך תא הכותרת ("מעל [___] דקות בדיווח בודד"). עמודת "התראה לעובד" מציגה "לא רלוונטי — התראה זו למנהל בלבד", עקבי עם `edit_request`. `saveNotificationSettings()` שולח `null` אם השדה ריק, ואת המספר אחרת.
